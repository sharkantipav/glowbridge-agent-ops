...
BANNED_PHRASE_PATTERNS: list[tuple[str, str]] = [
    (r"\bguarantee[ds]?\s+(?:bookings?|revenue|leads?|jobs?|customers?|sales?)\b",
        "guarantees outcomes"),
    ...
    (r"\b(?:guarantee|promise)\s+\$\d", "guarantees a dollar amount"),
    (r"\b(?:double|triple|10x|2x|3x)\s+your\s+(?:revenue|bookings|leads)\b",
        "guarantees a revenue multiple"),
    (r"\bget\s+\d+\s+(?:more\s+)?(?:bookings?|jobs?)\s+(?:per|a)\s+(?:day|week|month)\b",
        "guarantees a booking volume"),
    (r"\b100%\s+(?:booked|conversion|capture)\b", "claims 100% outcomes"),
    (r"\bnever\s+miss(?:es)?\s+a\s+(?:call|lead|job|booking)\b",
