...
def start() -> None:
    ...
    sched.add_job(_wrap(lambda: prospect.run(target=25), "prospect"),
                  CronTrigger(hour=7, minute=0), id="prospect_daily")
    sched.add_job(_wrap(lambda: research.run(limit=50), "research"),
                  CronTrigger(hour=7, minute=30), id="research_daily")
    sched.add_job(_wrap(lambda: outreach.run(limit=25), "outreach"),
